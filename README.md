...
1. To use the encoding function, the First argument is "E", the second argument is "S" if you want to show the result of methods, or “N” to only show the compression rate and rank. (Note: for any not text files (eg. image), the program will only show the compression rate and rank automatically, because the result of other kind of file is meaningless.)The third argument is the name of the input text file, and the fourth argument is the name of output file. In the following example, the program will encode the “test.txt” and show the result, then output an encoded file encode.bin”.
...
4. There are two sample inputs, in the “inefficient_input.txt”, which contains a string of non-repeating letters, methods will have a bad result of this sample input. In the “efficient_input.txt”, which contains the text of a long speech. Methods will have a good result for this sample input.
